...
function progressbar(text, time, anim)
	-- Load and play proper searching/stealing animation
	RequestAnimDict("mini@repair")
	while not HasAnimDictLoaded("mini@repair") do
		Wait(100)
	end
	TaskPlayAnim(PlayerPedId(), "mini@repair", "fixing_a_ped", 8.0, -8.0, -1, 16, 0, false, false, false)
	...
	if progressbartype == 'oxbar' then 
	  if lib.progressBar({ duration = time, label = text, useWhileDead = false, canCancel = true, disable = { car = true, move = true},}) then 
		-- Don't clear animation here - let minigame handle it
		return true
	  else
  ...
  function minigame(tier)
	...
	if Config.Tiergames[tier] == 'ps_circle' then
		...
		if GetResourceState('ps-ui') ~= 'started' then
			...
			-- Add error handling for the export
			local success_export, error_msg = pcall(function()
			...
			if not success_export then
				...
				if timeout >= 100 then
					print("^1[ERROR] ps_circle minigame timed out!")
					result = false
				else
					result = check
				end
   ...
   elseif Config.Tiergames[tier] == 'ox' then
		...
		if lib and lib.skillCheck then
			...
			print("^1[ERROR] ox_lib not found or skillCheck function missing")
	...
	elseif Config.Tiergames[tier] == 'none' then 
		...
		print("^1[ERROR] ritwik-houserobberies - Unknown minigame type: " .. tostring(Config.Tiergames[tier]) .. " for tier " .. tier)
		result = false
	end
   ...
   elseif Config.HouseHacks[tier] == 'ps_thermite' then
	...
	elseif Config.HouseHacks[tier] == 'none' then 
		return true			
		   ...
		   print"^1 SCRIPT ERROR: ritwik-houserobberies set your minigame with one of the options!"
	end
 end
...
function GetCops(number)
	...
	local amount = lib.callback.await('ritwik-houserobberies:server:GetCoppers', false)
...
if not minigame2(Config.Houses[k]['tier']) then TriggerServerEvent('ritwik-houserobbery:server:accessbreak', Config.Houses[k]['tier'], Config.LockpickItems[tier].item ) return end
	TriggerServerEvent('ritwik-houserobbery:server:enterHouse', k)
	SpawnHomeowner(k)
