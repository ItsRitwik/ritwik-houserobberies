...
if logs then 
    print'^2 Logs Enabled for ritwik-houserobberies'
    if logapi == nil then 
    ...
    print'^1 logs disabled for ritwik-houserobberies'
end
end)
...
function CheckDist(source, coords)
    ...
    if #(pcoords - coords) < 4.0 then
        ...
        DropPlayer(source, 'ritwik-houserobberies: You Were A Total Of ' .. #(pcoords - coords) .. ' Too Far Away To Trigger This Event') 
        return false
    end
end
...
lib.callback.register('ritwik-houserobberies:server:GetCoppers', function(source, cb, args)
    local amount = 0
...
CreateThread(function()
    local url = "https://raw.githubusercontent.com/Mustachedom/ritwik-houserobberies/main/version.txt"
    local version = GetResourceMetadata('ritwik-houserobberies', "version" )
